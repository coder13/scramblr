...
function FullCube_randomCube(){
	...
	shape = Shape_ShapeIdx[~~(randomFunc() * 3678)];
	corner = 0x01234567 << 1 | 0x11111111;
	edge = 0x01234567 << 1;
	n_corner = n_edge = 8;
	for (i=0; i<24; i++) {
		if (((shape >> i) & 1) == 0) {//edge
			rnd = ~~(randomFunc() * n_edge) << 2;
			FullCube_setPiece(f, 23-i, (edge >> rnd) & 0xf);
			m = (1 << rnd) - 1;
			edge = (edge & m) + ((edge >> 4) & ~m);
			--n_edge;
		} else {//corner
			rnd = ~~(randomFunc() * n_corner) << 2;
...
function binarySearch(sortedArray, key){
  ...
  var initialize = function(doneCallback, iniRandomSource, statusCallback) {

    iniRandomSource && setRandomFunc(iniRandomSource);
  ...
  const setRandomFunc = function(src) {
    randomFunc = src;
  };
  ...
  const getRandomScramble = function() {
    var randomState = getRandomPosition();
    var scrambleString = Search_solution(new Search_Search, randomState);

    return scrambleString;
  };

  Scramblers.register('sq1', randomSrc => initialize(() => null, randomSrc), '', getRandomScramble);
};
