const { ini, customScramble, getCustomScramble, rn } = require('./333.js');
...
module.exports = function (Scramblers) {
  Scramblers
    .register('c', src => ini(src), '', () => getCustomScramble({cp: [5,6,7]}))
    .register('333', src => ini(src), '', getRandomScramble)
    .register('edges', src => ini(src), '', getEdgeScramble)
    .register('LL', src => ini(src), '', getLLScramble)
    .register('LSLL', src => ini(src), '', getLSLLScramble)
    .register('ZZLSLL', src => ini(src), '- LSLL with EO solved', getZZLSLLScramble)
    .register('ZZL2SLL', src => ini(src), '', getZZL2SLLScramble)
    .register('ZBLL', src => ini(src), '', (args) => {
    ...
    .register('TRIZBLL', src => ini(src), `- Tripod ZBLL\n  args: ${Object.keys(coSubsets).join(', ')}`, (args) => {
      let s = rn(4);
    ...
    .register('PLL', src => ini(src), '', getPLLScramble)
    .register('ZZLL', src => ini(src), '', getZZLSScramble)
    .register('CMLL', src => ini(src), '', getCMLLScramble)
    .register('CMLLSune', src => ini(src), '', () => {
      let s = rn(2)+1;
    ...
    .register('LCCP', src => ini(src), '', (args) => {
