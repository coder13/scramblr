...
module.exports = function (Scramblers) {
  Scramblers
    ...
    .register('ZZLSLL', ini, getZZLSLLScramble)
    ...
    .register('TRIZBLL', ini, (args) => {
      ...
      let co = subset[rn(subset.length)];
      let cori = [0,0,0, ...co, 0].reverse();

      return getCustomScramble({
        ep: [EP.UF, EP.UR],
        cp: [CP.FLU, CP.FRU, CP.BRU],
        // co: [CO.FLU, CO.FRU, CO.BRU],
        cori: parseInt(cori.join(''),3)
      });
    })
    .register('2GLL', ini, (args) => {
      let co = getRandomCO(args);
